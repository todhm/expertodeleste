...
def parse_option_cases(pd: product_data.ProductData, driver: webdriver.Chrome) -> None:
    wrapper_elem = driver.find_element_by_xpath("//div[contains(@class, 'price-box-bundle')]")
    ...
    try:
        before_sales_price = float(wrapper_elem.find_element_by_xpath(
            ".//p[@class='list-price']//span[@class='price']"
        ).get_attribute('textContent').replace(',', '').replace("$", ''))
    except Exception:
        before_sale_price = price
    pd.beforeSalePrice = before_sales_price
    pd.price = price
    option_list = driver.find_elements_by_xpath("//div[@id='product-options-wrapper']//dl/dd[contains(@class, 'item')]")
    ...
    for sku_data in option_list:
        ...
        sku_id_elem = sku_data.find_element_by_xpath(".//span[contains(., 'ID:')]/*[1]")
    ...
    for base_set in base_set_compositions:
        ...
        name = base_set.get_attribute("textContent").replace(f'{default_count} x', '').strip()
        a_tag = base_set.find_elements_by_xpath('.//a/img')
        if a_tag:
            image = a_tag.get_attribute("src")
        else:
            image = list(filter(lambda x: name in x.productName.strip(), pd.additionalProductList))[0].thumbnailImage
...
def parse_furniturecart_data(driver: webdriver.Chrome, url: str, additional_product: Optional[product_data.AdditionalProductList] = None) -> product_data.ProductData:
    try:
    ...
    try:
        ...
        pd.productId = driver.find_element_by_xpath('//div[@class="sku" or contains(@class, "sku-social")]/meta[@itemprop="sku"]').get_attribute("content")
    except Exception:
